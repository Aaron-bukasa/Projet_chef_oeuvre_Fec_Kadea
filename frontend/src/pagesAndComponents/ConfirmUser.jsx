...
export default function confirmUser({setIsLogin}) {
    ...
    const validateCode = async(event) => {
        ...
        try {
          const response = await axios.post("http://localhost:3000/users/member/checkCode", {requestId: requestId, code:code});
    ...
    const handleResendCode = async() => {
        ...
        try {
          const response = await axios.post("http://localhost:3000/users/member/resendCodeConfirmation", {requestId: requestId});
  ...
  return (
    <div className="bg-bg_desktop flex flex-col justify-center items-center gap-y-8 p-3 min-h-screen roboto-regular sm:p-6 sm:gap-y-12 md:w-11/12 md:mx-auto lg:mx-auto lg:w-9/12 2xl:w-7/12 2xl:max-w-4xl">
    <div className="absolute top-0 left-0 w-full h-full z-[-999]">
      ...
      <div className="linear-bg rounded-lg sm:p-6">
        ...
        >
          ...
          Nous vous avons envoyé un code de confirmation par e-mail. <span className="font-semibold">Ce code est nécessaire</span> pour confirmer votre adresse e-mail. Veuillez le copier et coller dans le champ ci-dessous.{" "}
