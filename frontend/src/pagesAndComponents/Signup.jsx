...
export default function Signup({setIsLogin}) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/users/member/signup', {
  ...
  return (
    <div className="bg-slate-200 h-screen">
      <div className="relative px-[10%] py-12 md:py-24 bg-bg_desktop 2xl:px-[13%] flex flex-col gap-y-12 w-max mx-auto">
      <div>
        <img src={logoFec} alt="logo de la fec" />
      </div>
      <div>
      <form
        onSubmit={handleSubmit(onSubmit)}
        className="linear-bg border-2 text-white w-full px-3 py-4 rounded-xl sm:mb-6 md:mb-12 w-[768px] md:mx-auto flex flex-col gap-y-3 md:pb-12"
      >
