...
export default function NavbarUtilisateur() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await axios.post(`http://localhost:3000/suivi_utilisateur/${user.split(',')[0]}`, {
                email: user.split(',')[2]
            });
        ...
        <div className={`${isClick ? 'h-screen grid grid-cols-2 grid-rows-[max-content,max-content,max-content]' : 'h-auto'} text-xl flex justify-between items-center p-6 mb-2 shadow-lg shadow-opacity-75 shadow-color-gray-300 lg:text-lg lg:items-center lg:justify-between xl:text-xl 2xl:px-44`}>
          ...
          <ul className="flex flex-col gap-y-6 lg:gap-y-0 lg:flex-row lg:gap-x-6 lg:pl-5 xl:justify-between xl:pl-12 xl:ml-6 2xl:w-[20%] 2xl:pl-20 2xl:ml-12">
                ...
                <li onClick={handleClickLink} onMouseOver={() => isHover ? setIsHover(false) : setIsHover(true)}>
                    <Link to="profil" className='text-gray-400 flex items-center gax-x-6 login text-black'>
                        <svg xmlns="http://www.w3.org/2000/svg" className={`${isHover ? 'fill-[#4885ff]' : 'fill-black'}`} height="24" viewBox="0 -960 960 960" width="24">
                            <path d="M480-480q-66 0-113-47t-47-113q0-66 47-113t113-47q66 0 113 47t47 113q0 66-47 113t-113 47ZM160-160v-112q0-34 17.5-62.5T224-378q62-31 126-46.5T480-440q66 0 130 15.5T736-378q29 15 46.5 43.5T800-272v112H160Zm80-80h480v-32q0-11-5.5-20T700-306q-54-27-109-40.5T480-360q-56 0-111 13.5T260-306q-9 5-14.5 14t-5.5 20v32Zm240-320q33 0 56.5-23.5T560-640q0-33-23.5-56.5T480-720q-33 0-56.5 23.5T400-640q0 33 23.5 56.5T480-560Zm0-80Zm0 400Z"/>
                        </svg>
                        <div className={`${isHover ? 'text-[#4885ff]' : 'text-black font-bold'}`}>{user.split(',')[1]}</div>
                    </Link>
                </li>
