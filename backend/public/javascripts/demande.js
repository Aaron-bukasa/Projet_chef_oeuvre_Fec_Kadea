...
async function actionValider() {
    const postData = {
        requestId: supprimer.dataset.id,
    };
    ...
    try {
        ...
        if(response.status === 200) {
            window.localStorage.setItem("demandeDelete", 'Demande supprimée avec succèes');
            return window.location.href = '/demandes';
        }
    } catch (error) {
        return alert(error)
    }
...
function confirmation() {
    ...
    p.textContent = "Voulez-vous vraiment supprimer cette demande ?"
